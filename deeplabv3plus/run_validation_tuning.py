...
def evaluate_scores(scores_list, labels_list, metric="AUROC"):
    ...
    valid = np.isfinite(all_scores) & np.isfinite(all_labels)
    all_scores = all_scores[valid]
    all_labels = all_labels[valid].astype(np.uint8)
    if len(all_scores) == 0:
        return None
...
def run_method_on_list(model, galleries, image_paths, mask_dir, transform, method, max_size, **method_kwargs):
    ...
    for img_path in tqdm(image_paths, desc=f"{method}", leave=False):
        ...
        if (mask_img.size[0], mask_img.size[1]) != (sw, sh):
...
def main():
    ...
    parser.add_argument("--save-params", type=str, default=None,
                        help="Save best params to JSON (key=method). Merge with existing file.")
    ...
    # Mahalanobis++: only tune layers for which a gallery exists (avoids KeyError: 'pca')
    if method == "Mahalanobis++" and grid:
        has_layer3 = (
            "mahalanobis_layer3" in galleries
            and galleries["mahalanobis_layer3"]
            and "pca" in galleries.get("mahalanobis_layer3", {})
        )
        has_layer4 = (
            "mahalanobis_layer4" in galleries
            and galleries["mahalanobis_layer4"]
            and "pca" in galleries.get("mahalanobis_layer4", {})
        )
        grid = [
            p for p in grid
            if (p.get("layer") == "layer3" and has_layer3) or (p.get("layer") == "layer4" and has_layer4)
        ]
        if not grid:
            print(
                "Mahalanobis++ requires at least one of mahalanobis_gallery_layer3.pkl or "
                "mahalanobis_gallery_layer4.pkl in gallery_dir. Build with:\n"
                "  python build_feature_gallery_layer3.py   # layer3\n"
                "  python build_feature_gallery.py          # layer4"
            )
            return
        print(f"Mahalanobis++ grid filtered to available galleries: {len(grid)} runs (layer3={has_layer3}, layer4={has_layer4})")
    ...
    if args.save_params:
        import json
        save_path = os.path.abspath(args.save_params)
        all_params = {}
        if os.path.isfile(save_path):
            with open(save_path, "r") as f:
                all_params = json.load(f)
        all_params[method] = best_params
