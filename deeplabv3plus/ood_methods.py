...
def mahalanobis_score(features, gallery, alpha=0.01, n_components=None, target_size=None):
    ...
    # Require gallery with pca, means, cov (from mahalanobis_gallery_*.pkl)
    if not gallery or not isinstance(gallery, dict):
        raise ValueError("Mahalanobis requires gallery dict. Build with build_feature_gallery_balanced.py (bottleneck) or build_feature_gallery_layer3.py (layer3).")
    if 'pca' not in gallery or 'means' not in gallery or 'cov' not in gallery:
        raise ValueError(
            "Mahalanobis gallery must contain 'pca', 'means', 'cov'. "
            "For Mahalanobis++ use mahalanobis_gallery_layer4.pkl or mahalanobis_gallery_layer3.pkl. "
            "Build layer3 with build_feature_gallery_layer3.py; layer4 requires a layer4 gallery builder."
        )
    pca = gallery['pca']
    means = gallery['means']
...
def knn_score(features, gallery, k=50, target_size=None):
    ...
    # Chunk sizes chosen so matmul is ~100 MB to avoid OOM on shared GPUs (feat_chunk * gallery_chunk * 4 bytes)
    feat_chunk_size = 2500
    gallery_chunk_size = min(10000, N_gallery)
    n_pixels = features_flat.shape[0]
    if n_pixels > 50000:
        # High-res image: reduce feature chunk so matmul stays small
        feat_chunk_size = min(1000, max(500, 25_000_000 // max(gallery_chunk_size, 1)))
...
def dice_score(logits, p=90):
    ...
    Paper: keep top (1 - p/100) fraction of channels (sparse), then energy.
    
    Args:
        logits: [B, num_classes, H, W]
        p: Percentile; keep top (1 - p/100) of channels (default p=90 -> 10% channels).
...
def get_ood_score(logits, method='msp', features=None, model=None, input_tensor=None,
    ...
    if method == 'mahalanobis_plus_plus':
        if gallery is None or not gallery or not isinstance(gallery, dict) or 'pca' not in gallery:
            raise ValueError(
                "Mahalanobis++ requires mahalanobis_gallery_layer4.pkl or mahalanobis_gallery_layer3.pkl in gallery_dir. "
                "Build layer3 with: python build_feature_gallery_layer3.py. "
                "Layer4: build a layer4 gallery (see SegFormer build_feature_gallery_* for layer4) or use layer3."
            )
