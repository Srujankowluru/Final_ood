...
# Masks may use 128 for ignored pixels (not evaluated). Labels are float with nan = ignore.
IGNORE_MASK_VALUE = 128


def mask_image_to_ood_labels(mask_img: Image.Image, size_hw=None):
    """Convert L-mode mask to float labels: 0=ID, 1=OOD, nan=ignore (pixel value 128).
    Legacy masks with only {0,255} have no ignore pixels."""
    if size_hw is not None:
        mask_img = mask_img.resize((size_hw[1], size_hw[0]), Image.NEAREST)
    m = np.asarray(mask_img.convert("L"), dtype=np.float32)
    out = np.full(m.shape, np.nan, dtype=np.float32)
    valid = m != IGNORE_MASK_VALUE
    out[valid] = (m[valid] > 127).astype(np.float32)
    return out
...
def evaluate_method(scores_list, labels_list, method_name):
    ...
    # Remove ignored pixels (nan labels) and invalid scores
    valid_mask = np.isfinite(all_scores) & np.isfinite(all_labels)
    if not valid_mask.all():
        n_drop = int((~valid_mask).sum())
        if n_drop:
            print(f"  ⚠ {method_name}: Removing {n_drop} invalid/ignored pixels")
...
def main():
    ...
    parser.add_argument('--method-params', type=str, default=None,
                        help='JSON file of per-method hyperparams (e.g. from run_validation_tuning --save-params). Keys: method names.')
    ...
    method_params_loaded = {}
    if args.method_params and os.path.isfile(args.method_params):
        with open(args.method_params, 'r') as f:
            method_params_loaded = json.load(f)
        print(f"Loaded method params from {args.method_params} for: {list(method_params_loaded.keys())}")
    ...
    for method in methods_to_run:
        ...
        method_kwargs_extra = {}
        if method in method_params_loaded:
            method_kwargs_extra.update(method_params_loaded[method])
        if method.lower() == "actsub":
            method_kwargs_extra["lambda_"] = actsub_lambda
        if method.lower() in ("mahalanobis++", "mahalanobis_plus_plus") and "layer" not in method_kwargs_extra:
        ...
        for img_path in tqdm(image_paths, desc=method):
            ...
            mask_h, mask_w = mask_arr.shape[0], mask_arr.shape[1]
            if scores.shape[0] != mask_h or scores.shape[1] != mask_w:
                t = torch.from_numpy(np.asarray(scores, dtype=np.float32)).float().unsqueeze(0).unsqueeze(0)
                t = F.interpolate(t, size=(mask_h, mask_w), mode='bilinear', align_corners=False)
                scores = t.squeeze().cpu().numpy()
            mask_gt = mask_image_to_ood_labels(mask_img)
        ...
        if len(scores_list) > 0:
            ...
            for s, lbl in zip(scores_list, labels_list):
                ...
                flat_l = np.asarray(lbl, dtype=np.float64).flatten()
                id_mask = (flat_l == 0) & np.isfinite(flat_l) & np.isfinite(flat_s)
                if id_mask.any():
