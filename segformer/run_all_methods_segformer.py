...
# Add sibling DeepLab folder so we reuse ood_methods and evaluation logic
SEGFORMER_ROOT = os.path.dirname(os.path.abspath(__file__))
DEEPLAB_ROOT = os.path.join(os.path.dirname(SEGFORMER_ROOT), "deeplabv3plus")
...
def main():
    ...
    parser.add_argument("--method-params", type=str, default=None, help="JSON of per-method hyperparams (e.g. WildDash tuned)")
    ...
    if args.method_params and os.path.isfile(args.method_params):
        with open(args.method_params, "r") as f:
            method_params_loaded = _json_mod.load(f)
    ...
    # Auto-add gallery-dependent methods only in normal mode.
    # In --only-missing mode, keep the exact missing list to avoid rerunning kNN/VIM.
    ...
    for method in methods_to_run:
        ...
        for img_path in tqdm(image_paths, desc=method):
            ...
            mask_gt = mask_image_to_ood_labels(mask_resized)
